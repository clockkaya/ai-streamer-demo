...
class PersonaManager:
            ...
            if p_dir.is_dir():
                ...
                if config_path.exists():
                    try:
                        ...
                        bundle = self._load_persona(persona_id, config_path, p_dir)
    ...
    def _load_persona(self, persona_id: str, config_path: Path, p_dir: Path) -> PersonaBundle:
        ...
        if knowledge_dir.exists():
            faiss.load_directory(str(knowledge_dir))
        else:
            knowledge_dir.mkdir()
    ...
    def default_persona_id(self) -> str:
        """如果没有指定，默认返回存活的第一个。如果一个都没有，返回 bot_star（容错）。"""
        if "bot_star" in self._personas:
            return "bot_star"
        if self._personas:
